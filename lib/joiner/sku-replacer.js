...
function write(fileName, match, noMatch, data, lastLine = false) {
  const { outputDir, filePrefix, fileExtension, noMatchCount } = data;
  ...
  if (lastLine || noMatch.length > linesCount) {
    fs.appendFileSync(
      path.join(outputDir, `${filePrefix}${getBucket(noMatchCount, Math.random().toString())}-no-match.${fileExtension}`),
      noMatch.join("\n") + "\n"
    );
